...
def sample_noise(batch_size, dim, seed=None):
    ...
    out = torch.rand(batch_size, dim) * 2 - 1  # uniform in (-1, 1)
    return out
...
def discriminator_loss(logits_real, logits_fake):
    ...
    true_labels=torch.ones(logits_real.shape).type(logits_real.dtype)
    fake_labels=torch.zeros(logits_fake.shape).type(logits_fake.dtype)
    loss = bce_loss(logits_real, true_labels.view(-1)) + bce_loss(logits_fake, fake_labels.view(-1))
...
def generator_loss(logits_fake):
    ...
    true_labels=torch.ones(logits_fake.shape).type(logits_fake.dtype)
    loss = bce_loss(logits_fake, true_labels.view(-1))
